...
def model_conv_4244_test(rawdata):
    ...
    model = model_from_json(open('model_4244_conv_architecture.json').read())
    model.load_weights('model_4244_conv_weights.h5')
    ...
    for i in range(prediction.shape[0]):
        for j in range(prediction.shape[1]):
            temp = temp + abs(prediction[i][j] - dataY_Test[i][j]) / dataY_Test[i][j]
    error = temp / (prediction.shape[0] * prediction.shape[1])
    print("Model_4244_conv error: %.2f%%" % (error * 100))
